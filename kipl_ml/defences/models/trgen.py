...
class RNNCLF1(nn.Module):
    ...
    def __init__(
        ...
        if set(features).issubset(
            ...
            raise ValueError(
                f"Invalid set of feats. {'-'.join(features)}. "
                "Only TAM features are supported."
            )
    ...
    while True:
        ...
        if h is not None:
            ...
            if active_seqs.sum() != bs:
                raise ValueError(
                    "Initial hidden state must be provided when some sequences "
                    "are inactive."
                )
...
class RNNDefenceAgent(nn.Module):
    ...
    def _init_action_selection_prefer_wait(self, prefer_wait_bias: float) -> None:
        """Initialize selector logits to heavily prefer DO_NOTHING."""
