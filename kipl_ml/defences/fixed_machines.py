...
def load_machines(
    ...
    with open(deck_path, "r", encoding="utf-8") as fi:
        ...
        defenses = []
        while line != "":
            defenses.append(json.loads(line))
            line = fi.readline()
...
class _FixedMachine(_Def):
    def __init__(
        ...
        if (MAYBENOT := os.getenv("MAYBENOT")) is None:
            raise ValueError(
                "MAYBENOT not found in environment (deprecated: MACHINATION)"
            )
        ...
        self._machination(tmpfile_, seed)
    ...
    def _mlflow_log_params(self) -> dict[str, str]:
        ...
        d["cmd"] = " ".join(self.machination_args)
