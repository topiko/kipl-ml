...
class TestObsrlIntBinsConsistency(TestCase):
    ...
    def test_fd_uses_time_bins_not_times(self):
        ...
        fd, _, active = streamer.step([NoAction(time=0)])
    ...
    def test_actions_use_delay_bins_not_delay(self):
        """Actions enum should expose direction-specific delay actions."""
        self.assertTrue(hasattr(Actions, "DELAY_UP"))
        self.assertTrue(hasattr(Actions, "DELAY_DOWN"))
        self.assertFalse(hasattr(Actions, "DELAY_BINS"))

    def test_streaming_returns_time_bins(self):
        """Streaming streamer should return TIME_BINS, not TIMES."""
    ...
    def test_delay_uses_dedicated_duration_head(self):
        ...
        delay_lin = obs.actor["delay_bins_u"][-1]
        with torch.no_grad():
        ...
        self.assertIn(Actions.DELAY_UP, actions[0])
        self.assertEqual(int(actions[0][Actions.DELAY_UP].steps), 4)
